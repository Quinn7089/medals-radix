...
import { AArrowDown, AArrowUp, ArrowUp10, ArrowDown10 } from 'lucide-react';
...
function App() {
  ...
  const apiEndpoint = "https://olympicapi-cwh7cag2efc8awh4.centralus-01.azurewebsites.net/jwtapi/country";
  const hubEndpoint = "https://olympicapi-cwh7cag2efc8awh4.centralus-01.azurewebsites.net/medalsHub";
  const userEndpoint = "https://loginapi-eff4fydzeufagte3.centralus-01.azurewebsites.net/api/user/login";
 ...
 const [user, setUser] = useState({
  ...
  // const sortNumberAscending = [...countries].sort((a, b) => a.name.localeCompare(b.name));
  // const sortNumberDescending = [...countries].sort((a, b) => b.name.localeCompare(a.name));
  ...
  const sortMedalsAscending = () => {
    setSortBy("medalCount");
    setSortDirection("asc");
  }

  const sortMedalsDescending = () => {
    setSortBy("medalCount");
    setSortDirection("desc");
  }
  ...
  // Memoize the sorted countries to avoid sorting on every render
  const sortedCountries = useMemo(() => {
    ...
    } else if (sortBy === "medalCount") {
        const getTotalMedals = (country) => {
          let sum = 0;
          medals.current.forEach((medal) => {
            sum += country[medal.name].page_value;
          });
          return sum;
        };

        const aTotal = getTotalMedals(a);
        const bTotal = getTotalMedals(b);
       if (sortDirection === "asc") {
  ...
  return (
    <Theme appearance={appearance}>
      ...
      <Flex p="2" pl="8" className="fixedHeader" justify="between">
        ...
        <Button className="icon-button-CU" ml="6" onClick={sortCountriesAscending}> <AArrowUp /> </Button>
        <Button className="icon-button-CD" ml="2" onClick={sortCountriesDescending}> <AArrowDown /> </Button>
        <Button className="icon-button-MU" ml="6" onClick={sortMedalsAscending}> <ArrowUp10 /> </Button>
        <Button className="icon-button-MD" ml="2" onClick={sortMedalsDescending}> <ArrowDown10 /> </Button>
